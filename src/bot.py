...
class BotBase(commands.AutoShardedBot):
    ...
    def setup_logging(self, path):
        ...
        handler = RotatingFileHandler(
            filename=path, encoding="utf-8", mode="w", maxBytes=max_bytes, backupCount=3
        )
    ...
    def start_bot(self):
        ...
        try:
            ...
            self.logger.warning(
                "Unable to connect to database. You will not have access to some commands."
            )
...
async def on_message(message):
    ...
    if message.author.id == client.user.id:
        return
    if (
        message.author.id in json_config["blacklisted-users"]
        or message.guild.id in json_config["blacklisted-guilds"]
    ):
        return
    if re.fullmatch(r"<@(!)?790632534350233630>", message.content):
        await message.channel.send(
            f'Hello! I am {client.user.name}. Your prefix is "{client.prefixes.get(message.author.id, client.command_prefix)}". Use {client.prefixes.get(message.author.id, client.command_prefix)}help to get a list of commands.'
        )
