...
class Bookmarking(commands.Cog):
    ...
    async def _bookmark(self, ctx):
        await ctx.send(
            f'Did you mean: {self.bot.prefixes.get(ctx.author.id, "~")}bookmark add <id>?'
        )
    ...
    async def _pinned(self, ctx, args=""):
        ...
        if len(pins) > 3:
            confirm = await ctx.prompt(
                f"Holdup, you are about to bookmark {len(pins)} messages, do you wanna do this?"
            )
    ...
    async def _bookmarks(self, ctx, *, args: str = ""):
        ...
        bookmarks = await self.bot.db.fetch(
            "SELECT * from bookmarks WHERE bookmark_owner_id = $1", ctx.author.id
        )
        ...
        for bookmark in bookmarks:
            ...
            if not args.show_id:
                info = await self.bot.loop.run_in_executor(
                    ...
                    datetime.utcnow() - message_info["created_at"],
                )
            else:
        ...
        for page in paginator.pages:
            embed = discord.Embed(description=page, color=ctx.bot.embed_color)
            ...
            if dm:
                ...
                try:
                    await self.bot.wait_for(
                        "raw_reaction_add", check=check, timeout=120
                    )
    ...
    @_bookmark.command(
        name="pin", help="Pin a bookmark to the top of your bookmark list!"
    )
    ...
    async def _remove_bookmark(self, ctx, bookmark_id: int):
        if await self.bot.db.fetch(
            "SELECT FROM bookmarks WHERE database_id=$1 AND bookmark_owner_id=$2",
            bookmark_id,
            ctx.author.id,
        ):
            confirm = await ctx.prompt(
                "Are you sure you would like to delete this bookmark?"
            )
            ...
            await ctx.temp_send(
                "Could not find bookmark, are you sure it exists, or you own it?"
            )
    ...
    async def _clear(self, ctx):
        confirm = await ctx.prompt(
            "You are about to delete all of your bookmark! are you sure you would like to perform this action?"
        )
        if not confirm:
            await ctx.send("Aborting.")
            return
            ...
            await self.bot.db.execute(
                "DELETE FROM bookmarks WHERE bookmark_owner_id=$1", ctx.author.id
            )
