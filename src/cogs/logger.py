...
class Logging(commands.Cog, command_attrs=dict(hidden=True)):
    ...
    async def on_command(self, ctx):
        self.bot.loop.run_in_executor(
            ...
            f'Command: "{ctx.message.content}" executed in {ctx.guild.name} ({ctx.guild.id}) by {ctx.author.id}',
        )

    @commands.Cog.listener()
    async def on_guild_join(self, guild):
        self.bot.loop.run_in_executor(
            None, self.bot.logger.info, f"Joined guild: {guild.name} ({guild.id})"
        )

    @commands.Cog.listener()
    async def on_guild_remove(self, guild):
        self.bot.loop.run_in_executor(
            None, self.bot.logger.info, f"Left guild: {guild.name} ({guild.id})"
        )

    @commands.Cog.listener()
    async def on_shard_connect(self, shard):
        self.bot.loop.run_in_executor(
            None, self.bot.logger.info, f"Shard connected: {shard}"
        )
