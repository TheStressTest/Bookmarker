...
class DoHelp(commands.HelpCommand):
        ...
        for cog, command in mapping.items():
            ...
            if command_names:
                embed.add_field(
                    ...
                    value=f'`{await trim_message(", ".join(command_names), 20)}`',
                )

        embed.add_field(
            name="Cool links:",
            value="[`Support Server`](https://discord.gg/2FeXRTkXJP)\n[`Invite`](https://discord.com/api/oauth2/authorize?client_id=790632534350233630&permissions=67488832&scope=bot)\n[`Beta Invite`](https://discord.com/api/oauth2/authorize?client_id=811280192298811423&permissions=67488832&scope=bot)",
            inline=False,
        )
    ...
    async def send_group_help(self, group):
        embed = discord.Embed(title=group.name, color=self.context.bot.embed_color)
    ...
    async def send_cog_help(self, cog):
        embed = discord.Embed(
            title=cog.qualified_name, color=self.context.bot.embed_color
        )
        _commands = cog.get_commands()
        for command in cog.get_commands():
            embed.add_field(
                name=self.get_command_signature(command),
                value=f"`{command.brief if command.brief else command.help}`",
                inline=False,
            )
        channel = self.get_destination()
        await channel.send(embed=embed)
    ...
    async def command_not_found(self, string):
        ...
        _commands = extract(
            query=string, choices=command_names, limit=3, score_cutoff=70
        )
...
class Meta(commands.Cog, name="Config"):
    ...
    @commands.command(
        name="prefix",
        aliases=["setprefix"],
        help="Set your custom prefix using ~prefix <prefix>",
    )
