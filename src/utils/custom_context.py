...
class NewContext(commands.Context):
    ...
    async def clean_send(self, content=None, **kwargs):
        ...
        zero_space = "​"
        await self.send(
            content.replace("@everyone", f"@{zero_space}everyone").replace(
                "@here", f"@{zero_space}here"
            ),
            **kwargs,
        )
    ...
    async def delete_bookmark(self, _id):
        ...
        pool = self.bot.db
        await pool.execute(
            "DELETE FROM bookmarks WHERE database_id=$1", _id
        )  # database trigger should handle the rest.
    ...
    async def bookmark(self, message, args, cache=True, send_messages=True):
        ...
        try:
            await pool.execute(
                query, self.author.id, message.id, message.channel.id, args.hidden
            )
            if send_messages:
